...
int main(void) {
    ...
    // Pseudo-architecture
    void *guest_ram =
        mmap(NULL, GUEST_MEM, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    ...
    printf("  base=0x%llx limit=0x%x sel=0x%x type=0x%x p=%u dpl=%u s=%u g=%u "
           ...
           sregs.cs.present, sregs.cs.dpl, sregs.cs.s, sregs.cs.g, sregs.cs.db, sregs.cs.l,
           sregs.cs.avl, sregs.cs.unusable);

    printf("\nDS:\n");
    printf("  base=0x%llx limit=0x%x sel=0x%x type=0x%x p=%u dpl=%u s=%u g=%u "
           "db=%u l=%u avl=%u unusable=%u\n",
           (unsigned long long)sregs.ds.base, sregs.ds.limit, sregs.ds.selector, sregs.ds.type,
           sregs.ds.present, sregs.ds.dpl, sregs.ds.s, sregs.ds.g, sregs.ds.db, sregs.ds.l,
           sregs.ds.avl, sregs.ds.unusable);

    printf("\nES:\n");
    printf("  base=0x%llx limit=0x%x sel=0x%x type=0x%x p=%u dpl=%u s=%u g=%u "
           "db=%u l=%u avl=%u unusable=%u\n",
           (unsigned long long)sregs.es.base, sregs.es.limit, sregs.es.selector, sregs.es.type,
           sregs.es.present, sregs.es.dpl, sregs.es.s, sregs.es.g, sregs.es.db, sregs.es.l,
           sregs.es.avl, sregs.es.unusable);

    printf("\nFS:\n");
    printf("  base=0x%llx limit=0x%x sel=0x%x type=0x%x p=%u dpl=%u s=%u g=%u "
           "db=%u l=%u avl=%u unusable=%u\n",
           (unsigned long long)sregs.fs.base, sregs.fs.limit, sregs.fs.selector, sregs.fs.type,
           sregs.fs.present, sregs.fs.dpl, sregs.fs.s, sregs.fs.g, sregs.fs.db, sregs.fs.l,
           sregs.fs.avl, sregs.fs.unusable);

    printf("\nGS:\n");
    printf("  base=0x%llx limit=0x%x sel=0x%x type=0x%x p=%u dpl=%u s=%u g=%u "
           "db=%u l=%u avl=%u unusable=%u\n",
           (unsigned long long)sregs.gs.base, sregs.gs.limit, sregs.gs.selector, sregs.gs.type,
           sregs.gs.present, sregs.gs.dpl, sregs.gs.s, sregs.gs.g, sregs.gs.db, sregs.gs.l,
           sregs.gs.avl, sregs.gs.unusable);

    printf("\nSS:\n");
    printf("  base=0x%llx limit=0x%x sel=0x%x type=0x%x p=%u dpl=%u s=%u g=%u "
           "db=%u l=%u avl=%u unusable=%u\n",
           (unsigned long long)sregs.ss.base, sregs.ss.limit, sregs.ss.selector, sregs.ss.type,
           sregs.ss.present, sregs.ss.dpl, sregs.ss.s, sregs.ss.g, sregs.ss.db, sregs.ss.l,
           sregs.ss.avl, sregs.ss.unusable);

    printf("\nTR:\n");
    printf("  base=0x%llx limit=0x%x sel=0x%x type=0x%x p=%u dpl=%u s=%u g=%u "
           "db=%u l=%u avl=%u unusable=%u\n",
           (unsigned long long)sregs.tr.base, sregs.tr.limit, sregs.tr.selector, sregs.tr.type,
           sregs.tr.present, sregs.tr.dpl, sregs.tr.s, sregs.tr.g, sregs.tr.db, sregs.tr.l,
           sregs.tr.avl, sregs.tr.unusable);

    printf("\nLDT:\n");
    printf("  base=0x%llx limit=0x%x sel=0x%x type=0x%x p=%u dpl=%u s=%u g=%u "
           "db=%u l=%u avl=%u unusable=%u\n",
           (unsigned long long)sregs.ldt.base, sregs.ldt.limit, sregs.ldt.selector, sregs.ldt.type,
           sregs.ldt.present, sregs.ldt.dpl, sregs.ldt.s, sregs.ldt.g, sregs.ldt.db, sregs.ldt.l,
           sregs.ldt.avl, sregs.ldt.unusable);
    ...
    struct kvm_run *run =
        mmap(NULL, kvm_shared_space_size, PROT_READ | PROT_WRITE, MAP_SHARED, vcpu_fd, 0);
    ...
    while (1) {
        ...
        struct kvm_regs regs_exit;
        ioctl(vcpu_fd, KVM_GET_REGS, &regs_exit);

        printf("\n[VMEXIT] rip=0x%llx reason=%u\n", regs_exit.rip, run->exit_reason);
        ...
        case KVM_EXIT_FAIL_ENTRY:
            printf("[FAIL_ENTRY] hardware_reason=0x%llx\n",
                   run->fail_entry.hardware_entry_failure_reason);
            return 1;

        case KVM_EXIT_IO:
            if (run->io.direction == KVM_EXIT_IO_OUT && run->io.port == COM1_PORT) {

                uint8_t *data = (uint8_t *)run + run->io.data_offset;

                printf("[IO] port=0x%x size=%u data=0x%02x ('%c')\n", run->io.port, run->io.size,
                       *data, *data);
        ...
        default:
            printf("[UNHANDLED] exit_reason=%u\n", run->exit_reason);
            return 1;
        }
    }
